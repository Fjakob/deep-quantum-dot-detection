...
Specific Version for Statistical User Analysis and python2 compatible hopefully
"""
...
class App(tk.Tk, object):
    ...
    def createWidgets(self):
        ...
        NavigationToolbar2TkAgg(self.canvas, self.FrameFig)

        # Add user entry
        userFrame = tk.Frame(master=self.FrameLabel)
        tk.Label(master=userFrame, text="Enter user name: ").pack(side=tk.LEFT, fill=tk.BOTH)
        self.user_entry = tk.Entry(master=userFrame)
        self.user_entry.insert(0, "Username")
        self.user_entry.pack(side=tk.LEFT, fill=tk.BOTH, expand=1)

        # Create Peak Counter and Radio Buttons
        radioBtnFrame = tk.Frame(master=self.FrameLabel)

        # Peak Counter
        tk.Label(master=radioBtnFrame, text="Number of Peaks").grid(row=0, column=0)
        ...
        for idx, category in enumerate(categories):
            tk.Label(radioBtnFrame, text=category).grid(row=idx+1,column=0)
            for jdx, label in enumerate(textLabels):
                ttk.Radiobutton(radioBtnFrame, text=label, variable=category, value=numLabels[jdx]).grid(row=idx+1,column=jdx+1)  
        self.labels = categories

        # Create Buttons
        btnFrame = tk.Frame(master=self.FrameLabel)
        tk.Button(master=btnFrame, text="Submit", command=self.submit).pack(side=tk.RIGHT, fill=tk.BOTH)
        tk.Button(master=btnFrame, text="Skip", command=self.startFromTopDir).pack(side=tk.RIGHT, fill=tk.BOTH)

        # Add Frames to Label Frame
        userFrame.grid(row=0, column=0, sticky="nswe")
        radioBtnFrame.grid(row=1, column=0, sticky="nswe")
        btnFrame.grid(row=2, column=0, sticky="nswe")
    ...
    def plot_spectrum(self, wavelengths, amplitudes):
        """Plots a spectrum."""
        self.fig_ax.clear()
        self.fig_ax.plot(wavelengths, amplitudes)
        self.canvas.draw()
    ...
    def increase(self):
        ...
        self.lbl_value["text"] = "{}".format(value + 1)
