...
def pipeline(config_path):
    ...
    V_train = feature_extracter.extract_from_dataset(X_train, rescale=True)
    V_test  = feature_extracter.extract_from_dataset(X_test,  rescale=True)

    # 2) training stage
    neural_net.fit(V_train, Y_train, print_training_curve=True)
    y_pred_test = neural_net.predict(V_test)
    ...
    for idx in range(20):
        ...
        v_val = feature_extracter.extract_from_dataset(x_val, verbose=False)
        y_pred, sig = neural_net.predict(v_val, return_var=True)
        ...
        if idx==10:
            plt.subplot(2,1,1)
            plt.plot(X_test[idx])
            plt.title("True: {:.2f} Predicted: {:.2f} 95% confidence: ({:.2f}, {:.2f})".format(y_val, y_pred, conf_low, conf_high))
        if idx==14:
            plt.subplot(2,1,2)
            plt.plot(X_test[idx])
            plt.title("True: {:.2f} Predicted: {:.2f} 95% confidence: ({:.2f}, {:.2f})".format(y_val, y_pred, conf_low, conf_high))
