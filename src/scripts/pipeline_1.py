...
def pipeline(config_path):
    ...
    V = feature_extracter.extract_from_dataset(X, rescale=False, loss_window_size=1)
    V_val = feature_extracter.extract_from_dataset(X_val, rescale=False, loss_window_size=1)

    # 2) training stage
    neural_net.fit(V, Y)
    print(f"Validation Score: {neural_net.score(V_val, Y_val)}")
