...
def main():
    ...
    if retrain:
        ...
        reconstructor = Autoencoder(latent_dim=32)
        ...
        reconstructor.load_model('models/autoencoders/autoencoder32.pth')
    ...
    for iter, performance_dict in log.items():
        ...
        for feature, performance in performance_dict.items():
            ...
            plt.plot(iter, performance, '*' ,color=color, label=feature)
        if iter == 1:
            plt.legend()

    plt.plot(np.asarray(max_perf)[:,0], np.asarray(max_perf)[:,1], 'k')

    plt.grid()
    plt.ylim((0.65, 1))
    plt.xlabel('Iteration')
    plt.ylabel('R2 score')
    plt.title('Feature Forward Selection Algorithm')
    plt.show()
